...
const llenado = (peliculas) =>{
    let nuevo = []
    for (const key in peliculas) {
        nuevo.push(peliculas[key])
    }
    nuevo.map(pelicula=>{
        db.collection("nuevito").add({
            backgroundImg:pelicula.backgroundImg,
            cardImg:pelicula.cardImg,
            description:pelicula.description,
            subTitle:pelicula.subTitle,
            title:pelicula.title,
            titleImg:pelicula.titleImg,
            type:pelicula.type,
        })
    })
}
